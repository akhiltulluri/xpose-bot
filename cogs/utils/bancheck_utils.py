...
class BanCheckUtility:
    ...
    async def playerscan(self,playertag,league):
        ...
        if not vals and visited_banned_clans is None:
            emb = discord.Embed(title="Ban Check",description=f"Player {playername}[{playertag}] is not banned by {league.upper()} {emoji.Checkmark} and found no banned clan in player clan history!",colour=config.embed_color)
            return emb,embeds      
        for clan in visited_banned_clans:
            ...
            for explanation in explain_visited:
                ...
                current_player_history_explanation = current_player_history_explanation + f"**Role:** {explanation[2].capitalize()} **Between:** {start_dt} and {end_dt}"
                if (end - start).days > 0:
                    current_player_history_explanation = current_player_history_explanation + f" ({(end - start).days})\n"
                else:
            ...
            embed = discord.Embed(colour=config.embed_color,title=f"{clan_name}",description=description)
            embeds.append(embed)
        if not vals:
            if embeds:
                ...
                return embed,embeds
            emb = discord.Embed(color=config.embed_color,title='Ban Check',description=f"Player {playername}[{playertag}] is not banned by {league.upper()} {emoji.Checkmark} and didn't visit any banned clans previously!")
            return emb,[]               
    ...
    async def clanscan(self,clantag,league):
        ...
        if not vals:
            return discord.Embed(title=clanname,color=config.embed_color,description=f"Clan {clanname}[{clantag}] is not banned by {league.upper()} {emoji.Checkmark}")
