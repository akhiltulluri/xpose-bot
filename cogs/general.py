...
class General(commands.Cog):
    ...
    async def ping(self,ctx):
        ...
        embed = discord.Embed(title='Bot Latency', description=f'Pong! \U0001f3d3',colour=config.embed_color)
        msg1 = await ctx.send(embed=embed)
        await asyncio.sleep(3)
        new_embed= discord.Embed(title='Bot Latency', description=f'Pong! \U0001f3d3 Websocket Latency: {latency}ms',color=config.embed_color)
        await msg1.edit(embed=new_embed)

    @commands.command()
    async def uptime(self,ctx):
        """Shows Bot Uptime"""
        embed = discord.Embed(title='Uptime', description=f'{self.bot.uptime}',color=config.embed_color)
